...
class myTemped(chunked.Chunked):
    ...
    def is_starttag(self, word):
        ...
        if key in tconst.ORDINAL2  or key in tconst.ORDINAL1  or key in tconst.AWQAT or key in tconst.TEMPS :
           return True
...
###اوائل القرن
        ...
        if previous.isnumeric() and word in tconst.ORDINAL2:
            return True
        ##
        if previous in (u'قبل',u'بعد') and word in (u'استمرار'):
            return True
        ...
        if previous in  (u"في",) and word in tconst.ORDINAL:
            return True
